...
public class ModelAlumno { 
    ...
    Statement         st = null;
    ResultSet         rs = null;    
    String codigo, apellidos, nombres, carrera, DNI, direccion, correo;   
    ...
    public boolean registrarAlumno(ModelAlumno alumno) {
        String query = "INSERT INTO alumno (apellidos, nombres, id_carrera, DNI, direccion, usuario, contraseña, id_alumno) VALUES (?, ?, ?, ?, ?, ?, NULL, ?)";
    ...
    public boolean modificarAlumno(ModelAlumno alumno) {
        String query = "UPDATE alumno SET apellidos = ?, nombres = ?, id_carrera = ?, DNI = ?, direccion = ? WHERE id_alumno = ?";        
        try (Connection con = ConexionBD.getConexionBD();
            PreparedStatement ps = con.prepareStatement(query)) {    
            ps.setString(1, alumno.getApellidos());        
            ps.setString(2, alumno.getNombres());
